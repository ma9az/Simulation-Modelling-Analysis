...
const urlSeats="images/Chair-icon.png"
...
var srow=areas[0].startRow;
var scol=areas[0].startCol-1;
var nrow=areas[0].numRows-1;
var ncol=areas[0].numCols;
...
function range(start, end, step = 1) {
  ...
  var rowSeats = range(srow, srow+nrow, 1);
  var colSeats = range(scol+1, scol+ncol, 1);
 
//Create all possible combinations of building coordinates
  for (i = 0; i < rowSeats.length; i++) {
	for (j = 0; j < colSeats.length; j++){
		var newseat ={"row":rowSeats[i], "col":colSeats[j], "filled":false};
	    Seats.push(newseat); 
	}
  }
...
function updateSurface(){
	...
	var allseats = surface.selectAll(".Seats").data(Seats);
...
function updatePatient(patientIndex){
	...
	var seatsAvailable = Seats.filter(function(d){return d.filled==false;});
	...
	switch(state){
		...
		case UNTREATED:
			if (hasArrived){
				...
				var randomWaitingSeat = Math.floor(Math.random()*seatsAvailable.length);
				// pick a random spot in the waiting area to queue
				patient.target.row =  seatsAvailable[randomWaitingSeat].row;
				patient.target.col =  seatsAvailable[randomWaitingSeat].col;
				var waitingSeatIndex = Seats.findIndex(x => x == seatsAvailable[randomWaitingSeat]);
				Seats[waitingSeatIndex].filled=true;
				...
				else {patient.state=DISCHARGED; 
					   patient.target.row = 1;
					   patient.target.col = maxCols;}
			}
		break;
		case WAITING:
			var makeEmpty=Seats.filter(function(d){return d.row ==patient.target.row && d.col==patient.target.col;});
			if (makeEmpty.length>0){
				var oldSeatIndex = Seats.findIndex(x => x == makeEmpty[0]);
				Seats[oldSeatIndex].filled=false;
